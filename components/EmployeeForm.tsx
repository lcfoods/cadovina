  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      const index = parseInt(
        (e.target as HTMLElement).getAttribute('data-index') || '-1',
        10
      );
    ...
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 flex flex-col h-full overflow-hidden relative">
      ...
      <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center sticky top-0 z-10">
        <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
          <span className="w-2 h-6 bg-cadovina-600 rounded-sm inline-block" />
        ...
        <div className="flex gap-3">
          {!initialData && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-md hover:bg-indigo-100 border border-indigo-200 text-sm font-medium"
            >
              <Wand2 size={16} />
              AI Scan CV
            </button>
          ...
          <button
            onClick={onCancel}
            className="px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-100 border border-gray-300 text-sm font-medium"
          >
            Đóng
          </button>
        </div>
            ...
            <div className="bg-gray-50 p-4 rounded border border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({
                        ...p,
                        addressLevel: e.target.checked ? 2 : 3,
                        ward: '',
                        district: '',
                      }))
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <FormInput
                  ...
                  disabled={
                    !formData.province || formData.addressLevel === 2
                  }
                  error={errors.district}
                  required={formData.addressLevel === 3}
                  className={
                    formData.addressLevel === 2 ? 'opacity-80' : ''
                  }
                />
                <FormInput
                  index={12}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="flex flex-col">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className={`w-full px-3 py-2 bg-white border rounded-md text-sm shadow-sm focus:ring-2 focus:ring-cadovina-500 ${
                ...
                {errors.department && (
                  <span className="mt-1 text-xs text-red-500 italic">
                    {errors.department}
                  </span>
                )}
              </div>
              ...
              <div className="relative">
                ...
                {formData.resignationDate &&
                  new Date(formData.resignationDate) <= new Date() && (
                    <div className="mt-2 p-2 bg-orange-50 border border-orange-200 rounded text-xs text-orange-800 flex items-start gap-2 animate-pulse">
                      ...
                      />
                      <span>
                        <strong>Cảnh báo:</strong> Nhân viên sẽ chuyển sang
                        trạng thái{' '}
      ...
      <div className="px-6 py-4 border-t border-gray-200 bg-white flex justify-end gap-3">
        <button
          type="button"
          onClick={onCancel}
          className="px-6 py-2 rounded-md text-gray-700 font-medium hover:bg-gray-100 border border-gray-300"
        >
          Hủy bỏ
        </button>
        <button
          ...
          disabled={isSaving}
          className="flex items-center gap-2 px-6 py-2 rounded-md bg-cadovina-600 text-white font-medium hover:bg-cadovina-700 shadow-md"
        >
          {isSaving ? (
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-cadovina-600 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="font-bold text-lg flex items-center gap-2">
                <FileText size={20} /> Xác nhận thông tin
              </h3>
              <button
                onClick={() => setShowConfirmModal(false)}
                className="hover:bg-cadovina-700 rounded p-1"
              >
                <X size={20} />
              </button>
            </div>
            <div className="p-6 overflow-y-auto">
              <p className="mb-4 text-gray-600 text-sm">
                Vui lòng kiểm tra lại thông tin trước khi lưu chính thức vào hệ
                thống.
              </p>
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-sm">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium text-gray-800 flex items-center gap-2">
                    <Calendar size={14} /> {formData.startDate}
                  </p>
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase">
                  ...
                  <p className="font-medium text-gray-800">
                    {formData.salary.toLocaleString('vi-VN')} VNĐ
                  </p>
                </div>
            ...
            <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
              <button
                onClick={() => setShowConfirmModal(false)}
                className="px-4 py-2 border border-gray-300 rounded text-gray-700 font-medium hover:bg-white transition-colors"
              >
                Quay lại sửa
              </button>
              <button
                onClick={handleConfirmSave}
                className="px-6 py-2 bg-green-600 text-white rounded font-bold hover:bg-green-700 shadow-md flex items-center gap-2 transition-transform active:scale-95"
              >
