...
import { LayoutDashboard, Settings, Menu, Search, Bell, UserCheck, Users } from 'lucide-react';
    ...
    <div className="flex h-screen bg-gray-100 text-gray-800 font-sans overflow-hidden">
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shadow-xl z-20">
        ...
        <nav className="flex-1 py-6 px-2 space-y-1">
           <button onClick={()=>setView('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 border-l-4 transition-all ${view==='dashboard'?'bg-slate-800 border-red-500 text-white':'border-transparent text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
                <LayoutDashboard size={20}/> Tổng quan
            </button>
           <button onClick={()=>setView('employee-list')} className={`w-full flex items-center gap-3 px-4 py-3 border-l-4 transition-all ${view.startsWith('employee')?'bg-slate-800 border-red-500 text-white':'border-transparent text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
                <Users size={20}/> Nhân viên
            </button>
           <button onClick={()=>setView('recruitment')} className={`w-full flex items-center gap-3 px-4 py-3 border-l-4 transition-all ${view==='recruitment'?'bg-slate-800 border-red-500 text-white':'border-transparent text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
                <UserCheck size={20}/> Tuyển dụng
            </button>
           <button onClick={()=>setView('config')} className={`w-full flex items-center gap-3 px-4 py-3 border-l-4 transition-all ${view==='config'?'bg-slate-800 border-red-500 text-white':'border-transparent text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
                <Settings size={20}/> Cấu hình
            </button>
        </nav>
        <div className="p-4 bg-slate-800 text-xs text-slate-400 text-center">v1.0.5 (Sheet Sync)</div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-50">
        ...
        <div className="flex-1 overflow-hidden relative">
             {view === 'dashboard' && <Dashboard employees={employees} onViewEmployee={(e)=>{setSelectedEmp(e); setView('employee-form')}} />}
             ...
             {view === 'employee-form' && (
                <div className="h-full p-6">
                    <EmployeeForm 
                        ...
                        departments={depts} positions={positions} 
             ...
             {view === 'recruitment' && (
                <RecruitmentManager 
                    ...
                    onUpdateCandidates={(d) => updateAndSync('CANDIDATES', d, setCandidates)} 
                    onPromoteToEmployee={handlePromote} 
                    departments={depts} positions={positions} existingEmployees={employees} 
                />
             )}
